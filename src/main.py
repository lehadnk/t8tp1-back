...
from src.dto.schemas import User, ChangeUser, UserAuthenticationResponse, UserAuthenticationRequest
...
async def get_user_list(page: int = 1, page_size: int = 10, db_session: Session = Depends(get_db_session), user: User = Depends(require_admin_authorization)):
    users = storage.get_user_list(db_session, page, page_size)
    return users

@app.post("/users/")
async def save_user(user: ChangeUser, db_session: Session = Depends(get_db_session)):
...
async def authenticate_user(request: UserAuthenticationRequest, db_session: Session = Depends(get_db_session)):
    ...
    return UserAuthenticationResponse(auth_token='123')
